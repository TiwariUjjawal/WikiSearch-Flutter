...
Future<List<Page>> getRandomList(query, limit) async {
  var jsonData;

  String space = ' ';
  if (query.length >= 3) {
    for (int i = 1; i < query.length; i++) {
      if (query[i] == space && query[i - 1] != space && query[i + 1] != space) {
        query =
            query.substring(0, i) + '_' + query.substring(i + 1, query.length);
      }
    }
  }
